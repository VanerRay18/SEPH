      ...
      this.NominaBecService.getPreAnexo5(this.nominaId, ordinaria).subscribe({
        next: async response => {
          if (response && response.data && Array.isArray(response.data)) {
            ...
            // ---- CREAR HOJA "RESUMEN" ----
            const resumenHeaders = ['Datos', 'Valores'];
            const resumenData = [
              ['Clabes', this.resumen.clabes],
              ['Plazas', this.resumen.plazas],
              ['Deducciones', this.toNumber(this.resumen.deducciones)],
              ['Personas', this.resumen.personas],
              ['Percepciones', this.toNumber(this.resumen.percepciones)],
              ['Líquido', this.toNumber(this.resumen.liquido)]
            ];

            // Crear hoja de Excel para Resumen
            const worksheetResumen: XLSX.WorkSheet = XLSX.utils.aoa_to_sheet([resumenHeaders, ...resumenData]);
            worksheetResumen['!cols'] = [{ wpx: 120 }, { wpx: 150 }]; // Ajustar ancho de columnas
